  ...
  buttonSubscriptionShow(plan, email, subscription){
    if(!subscription){
      return ( <a type="submit" onClick={()=>{
        ...
        }} className={"btn "+Config.designSettings.submitButtonClass}>Purchase now</a>
      )
    }else{
      if(subscription.status != "deleted" && plan.id+"" === subscription.subscription_plan_id+""){
        return (
          <a type="submit" className={"btn "+Config.designSettings.submitButtonClass} disabled><i className="material-icons">done</i> Current plan</a>
        )
      }else{
        return ( <a type="submit" onClick={()=>{
          window.openCheckout(plan.id, email, true);
          }} className={"btn "+Config.designSettings.submitButtonClass}>{subscription.status == "deleted"?"Purchase now":"Switch"}</a>
        )
      }
    }
  }
  ...
  setUpSubscriptionOptions(subscription){
      ...
      return (
        <div className="col-md-12">
          <a href={subscription.update_url} target="_blank" className={"btn "+Config.designSettings.buttonInfoClass}>Update Billing Info</a>
          <a  href={subscription.cancel_url} target="_blank"   className={"btn "+Config.designSettings.submitButtonClass}>Cancel Subscription</a>
        </div>
      )}else{
        return (<div></div>)
      }
          ...
          <div className="row">   
            <div className="col-md-8 col-md-offset-2">
              ...
              <SkyLight
                  ...
                  title={this.state.planName+' Plan Features'}
